...
const ChatBox = () => {
    ...
    const { currentChats, messages, isMessagesLoading, messagesError, sendTextMessage } = useContext(ChatContext);
        ...
        <>
            <Stack gap={4} className='chat-box'>
                ...
                <Stack gap={3} className='messages'>
                    {messages && messages.map((message, index) => (<Stack key={index} className={`${message?.senderId === user?._id ?
                ...
                <Stack direction='horizontal' gap={3} className='chat-input flex-grow-0'>
                    ...
                    <button className='send-btn' 
                        onClick={()=> sendTextMessage(textMessage, user, currentChats._id, setTextMessage)}>
